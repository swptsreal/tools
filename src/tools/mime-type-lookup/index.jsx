...
import { Clipboard, FileType } from 'lucide-react'
...
export default function MimeTypeLookupTool(){const [query,setQuery]=useDraft(toolId,mimeTypeLookupExample); const matches=rows.filter((row)=>row.join(' ').toLowerCase().includes(query.toLowerCase().replace(/^\./,''))); const output=matches.map((r)=>r.join(' | ')).join('\n'); const copy=async()=>{const res=await copyText(output); message[res.ok?'success':'warning'](res.message)}; const actions=useMemo(()=><><Button icon={<FileType size={16}/>} type="primary">Lookup</Button><Button icon={<Clipboard size={16}/>} onClick={copy}>Copy</Button><RevertExample onClick={()=>setQuery(mimeTypeLookupExample)}/></>,[output]); useToolActions(actions); return <div className="tool-page developer-tool-page"><SplitWorkspace leftToolbar={<><label className="tool-function-label" htmlFor="mime-search">Search MIME</label><Input id="mime-search" aria-label="Search MIME" value={query} onChange={(e)=>setQuery(e.target.value)} /></>} left={<div className="dev-result"><div className="dev-card"><p>Search by extension, MIME type, or description.</p></div></div>} right={<div className="dev-result">{matches.map(([ext,mime,label])=><div className="dev-card" key={ext}><h3>{ext}</h3><p className="dev-mono">{mime}</p><p>{label}</p></div>)}</div>} /></div>}
